...
import {
  SetPriceAlertModal,
  type AlertTarget,
} from "./components/SetPriceAlertModal";
...
export default function App() {
        ...
        if (fr) {
          ...
          if (firstOpt) {
            ...
            const dep =
              ...
              (firstOpt as { departure_airport?: { id?: string } })
  ...
  const chipBase =
    "px-3 py-[0.35rem] text-[0.78rem] rounded-full border border-app-border bg-app-surface text-app-text-muted cursor-pointer hover:border-app-accent hover:text-app-accent hover:bg-app-accent/15 transition-all whitespace-nowrap";
  ...
  return (
    <div className="flex min-h-screen w-full bg-app-bg text-app-text max-[680px]:flex-col">
      ...
      {sidebarOpen && (
        <>
          <div
            className="hidden max-[680px]:block fixed inset-0 bg-black/50 z-[49]"
      ...
      <main className="flex-1 min-w-0 max-w-[min(960px,96vw)] mx-auto px-5 py-4 pb-6 flex flex-col gap-4 max-[680px]:max-w-full max-[680px]:px-4">
        ...
        <header className="flex items-center gap-2.5 pt-3">
          ...
          <p className="text-[0.8rem] text-app-text-muted m-0">
        ...
        <div className="flex flex-col gap-3">
          <div
            className="flex flex-col gap-4 min-h-[200px] max-h-[68vh] overflow-y-auto p-5 bg-app-surface rounded-2xl border border-app-border scroll-smooth"
            role="log"
            aria-live="polite"
          >
            {/* Empty state */}
            {isEmptyState && (
              <div className="flex flex-col items-center justify-center text-center gap-3 py-10 flex-1">
                <PlaneSVG large />
                <h2 className="text-[1.3rem] font-bold m-0 text-app-text">
            ...
            {messages.map((m, i) => (
              <div
                key={i}
                className={`flex flex-col gap-1.5 max-w-[95%] ${m.role === "user" ? "self-end" : "self-start max-w-full"}`}
              >
          ...
          {error && (
            <div
              className="flex items-center justify-between gap-2 px-3.5 py-[0.65rem] bg-app-red/10 border border-app-red/30 rounded-lg text-[0.875rem] text-app-red"
              role="alert"
            >
              <span>{error}</span>
              <button
                ...
                className="bg-transparent border-none text-app-red cursor-pointer text-xl leading-none p-0 opacity-70 hover:opacity-100"
              >
                ×
              </button>
            </div>
...
function PriceHistoryRouteForm({
  ...
  const inputCls =
    "px-[0.9rem] py-[0.6rem] rounded-lg border border-app-border bg-app-surface text-app-text text-[0.9rem] focus:outline-none focus:border-app-accent tracking-[0.08em] font-semibold uppercase placeholder:text-app-text-muted";
      ...
      <div className="flex justify-between items-start gap-3">
        <div>
          ...
          <p className="text-[0.78rem] text-app-text-muted mt-0.5 mb-0">
            Enter a route to view historical prices
          </p>
        </div>
        <button
          type="button"
...
function PlaneSVG({ large }: { large?: boolean }) {
  return (
    <svg
      className={
        large
          ? "w-12 h-12 text-app-accent opacity-60"
          : "w-5 h-5 text-app-accent"
      }
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden
