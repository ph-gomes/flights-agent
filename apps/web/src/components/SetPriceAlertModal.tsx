...
export function SetPriceAlertModal({ target, onClose }: SetPriceAlertModalProps) {
  ...
  const inputCls = "w-full px-[0.9rem] py-[0.6rem] rounded-lg border border-app-border bg-app-surface text-app-text text-[0.9rem] transition-colors focus:outline-none focus:border-app-accent placeholder:text-app-text-muted";

  return (
    <>
      {/* Backdrop */}
      <div className="fixed inset-0 bg-black/60 backdrop-blur-[2px] z-10" onClick={onClose} aria-hidden />
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[11] w-[min(480px,94vw)] max-h-[88vh] overflow-y-auto rounded-[14px]">
        <div className="bg-[#18181e] border border-app-border rounded-[14px] p-5 flex flex-col gap-4" role="dialog" aria-label="Set price alert">

          {/* Header */}
          <div className="flex justify-between items-start gap-3">
            <div>
              <h3 className="text-[1.1rem] font-bold m-0">🔔 Set Price Alert</h3>
              <p className="text-[0.78rem] text-app-text-muted mt-0.5 mb-0">
                ...
                {target.currentPrice != null && (
                  <> · currently <strong className="text-app-text">${target.currentPrice.toLocaleString()}</strong></>
                )}
              </p>
            </div>
          ...
          {success ? (
            <div className="flex flex-col items-center gap-3 py-4 text-center">
              <span className="flex items-center justify-center w-10 h-10 rounded-full bg-app-green/15 text-app-green text-xl font-bold">✓</span>
              <p className="text-[0.9rem] text-app-text-muted max-w-[32ch] leading-relaxed m-0">
                Alert set! We'll notify <strong className="text-app-text">{email}</strong> when the price drops to{" "}
                <strong className="text-app-text">${Number(targetPrice).toLocaleString()}</strong> or below.
              </p>
            ...
            <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
              <label className="flex flex-col gap-1.5">
                <span className="text-[0.8rem] font-semibold text-app-text-muted uppercase tracking-[0.04em]">Notify me at</span>
                <input type="email" className={inputCls} placeholder="you@example.com"
                  value={email} onChange={(e) => setEmail(e.target.value)} required autoFocus />
              </label>

              <label className="flex flex-col gap-1.5">
                <span className="text-[0.8rem] font-semibold text-app-text-muted uppercase tracking-[0.04em]">When price drops to or below</span>
                <div className="flex items-center gap-2">
                  <span className="text-[1rem] font-bold text-app-text-muted">$</span>
                  <input type="number" min="1" step="1" className={`${inputCls} flex-1`} placeholder="e.g. 350"
                    value={targetPrice} onChange={(e) => setTargetPrice(e.target.value)} required />
                </div>
              </label>

              {error && <p className="text-app-red text-[0.9rem] m-0">{error}</p>}

              <button type="submit" disabled={creating || !email || !targetPrice}
