...
(async () => {
  ...
  try {
    ...
    const heading = await page
      .getByRole("heading", { name: /where do you want/i })
    ...
    if (!heading?.toLowerCase().includes("where")) {
      const anyHeading = await page
        ...
        .catch(() => null);
      if (anyHeading?.toLowerCase().includes("where"))
        ok("TC-03", "Empty state with heading");
      else
        fail(
          "TC-03",
          `Expected welcome heading, got: ${heading ?? anyHeading ?? "none"}`,
        );
    } else {
      ok("TC-03", "Empty state with heading");
    }
    const firstChip = page.getByRole("button", {
      name: "Flights from NYC to London next weekend",
    });
    ...
    const typingVisible = await page
      ...
      .catch(() => false);
    if (typingVisible)
      ok("TC-03", "Chip sent, search started (typing indicator)");
    ...
    const cards = flightSection(page).getByRole("button", {
    ...
    const lowestPrice = flightSection(page)
      .locator("strong.text-app-green")
      .first();
    ...
    await sendMessage(
      page,
      "Round trip NYC to London, depart April 5, return April 12",
    );
    await page.waitForTimeout(500);
    try {
      await flightSection(page)
        .last()
        .waitFor({ state: "visible", timeout: TIMEOUT });
      const roundTripLabel = flightSection(page)
        .last()
        .getByText("round trip", { exact: false })
        .first();
      if (await roundTripLabel.isVisible().catch(() => false)) {
        ...
        skip(
          "TC-02",
          "Round trip label not found in DOM (may still be correct)",
        );
      }
    } catch (e) {
    ...
    const selectOutboundBtn = page
      .getByRole("button", { name: "Select outbound" })
      .first();
    const outboundVisible = await selectOutboundBtn
    ...
    if (outboundVisible) {
      ...
      const returnHeading = page.getByRole("heading", {
        name: "Return flights",
      });
      const returnVisible = await returnHeading
        .waitFor({ state: "visible", timeout: 15000 })
        .then(() => true)
        .catch(() => false);
      if (returnVisible) {
        ok("TC-02b", "Return flights section appeared after Select outbound");
        const selectReturnBtn = page
          .getByRole("button", { name: "Select return" })
          .first();
        const returnBtnVisible = await selectReturnBtn
        ...
        if (returnBtnVisible) {
          ...
          if (await selectedMsg.isVisible().catch(() => false))
            ok("TC-02b", "Return flight selected confirmation");
        } else {
          skip(
            "TC-02b",
            "Select return button not found (may still be loading)",
          );
        }
      } else {
        skip(
          "TC-02b",
          "Return flights section did not appear (API may not return departure_token)",
        );
      }
      ...
      fail(
        "TC-02b",
        "Select outbound button not found (round-trip outbound mode)",
      );
    }
    ...
    if (secondTime < firstTime * 0.5) {
      ...
      skip(
        "TC-08",
        `Timing not clearly faster: ${secondTime}ms vs ${firstTime}ms`,
      );
    }
    ...
    try {
      await flightSection(page)
        .last()
        .waitFor({ state: "visible", timeout: TIMEOUT });
      const nonstop = flightSection(page)
        .last()
        .getByText("Nonstop", { exact: true })
        .first();
    ...
    const historyChip = page
      .getByRole("button", { name: /price history/i })
      .first();
    const chipVisible = await historyChip
    ...
    if (chipVisible) {
      ...
      fail(
        "TC-12",
        "Price history chip not found (route chip appears after results)",
      );
    }
    ...
    skip(
      "TC-14",
      "No UI to open route form without prior search in current app",
    );
    ...
    await flightSection(page)
      .first()
      .getByRole("button", { name: /expand details|collapse details/ })
      .first()
      .click();
    await page.waitForTimeout(200);
    const setAlertBtn = flightSection(page)
      .first()
      .getByRole("button", { name: /Set alert/i })
      .first();
    ...
    if (await alertDialog.isVisible().catch(() => false)) {
      ...
      if (await title.isVisible().catch(() => false))
        ok("TC-16", "Title and route shown");
    } else {
    ...
    const setAlertSubmit = alertDialog.getByRole("button", {
      name: "Set Alert",
    });
    ...
    try {
      const alertsRes = await page.request.get(
        `${BASE}/api/price-alerts?email=qa@example.com`,
      );
      ...
      const match = list.find(
        (a) =>
          norm(a).dep && norm(a).arr && norm(a).active && norm(a).notTriggered,
      );
      const anyActive = list.some(
        (a) =>
          a.status === "active" && (a.triggeredAt ?? a.triggered_at) == null,
      );
    ...
    const emptyHeading = await page
      ...
      .catch(() => null);
    const sidebarSessions = page
      .getByRole("listitem")
      .filter({ has: page.getByText(/New conversation|conversation/i) });
    ...
    const firstSession = page
      .getByRole("button", { name: /New conversation|conversation/i })
      .first();
    ...
    if (deleteVisible) {
      ...
      skip(
        "TC-23",
        "Delete button not visible on hover (may need manual check)",
      );
    }
